...
void setBoundaryValues( SuperLattice3D<T, NSDESCRIPTOR>& sLatticeNS,
  ...
  if ( iT == 0 ) {
    ...
    // INLET
    std::vector<T> velocityInlet( 3,T() );
    velocityInlet[0]=0.0;
    velocityInlet[1]=0.0;
    velocityInlet[2]=-0.005;

    AnalyticalConst3D<T,T> Velocity0( velocityInlet );
    sLatticeNS.defineU( superGeometry, 3, Velocity0 );

    // OUTLET 1
    std::vector<T> velocityOutlet1( 3,T() );
    velocityOutlet1[0]=-0.001215284;
    velocityOutlet1[1]=0.0;
    velocityOutlet1[2]=-0.002184739;
    
    AnalyticalConst3D<T,T> Velocity1( velocityOutlet1 );
    sLatticeNS.defineU( superGeometry, 4, Velocity1 );

    // OUTLET 2
    std::vector<T> velocityOutlet2( 3,T() );
    velocityOutlet2[0]=0.001215284;
    velocityOutlet2[1]=0.0;
    velocityOutlet2[2]=-0.002184739;
    
    AnalyticalConst3D<T,T> Velocity2( velocityOutlet2 );
    sLatticeNS.defineU( superGeometry, 5, Velocity2 );
