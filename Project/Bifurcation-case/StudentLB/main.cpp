...
//******************************************************************************
    ...
    for ( unsigned Step = 1; Step <= TimeSteps; ++Step ) {
        ...
        for ( unsigned i = 0; i < CommunicationBuffers.size(); ++i ) {
            ...
            // Send-receive operation ( the communication subsystem)  takes care 
            // of the issue of preventring cyclic dependencies
            MPI_Sendrecv( CommunicationBuffers[ i ].getProtocol( collideField ),
                          CommunicationBuffers[ i ].getBufferSize(),
                          MPI_DOUBLE,
                          CommunicationBuffers[ i ].getTragetCpu(),
                          TAG,
    ...
    /*
		...
		BoundaryConditions[ 0 ]->Data[ 0 ] = 0.0;
		BoundaryConditions[ 0 ]->Data[ 1 ] = 0.0;
		BoundaryConditions[ 0 ]->Data[ 2 ] = NewInletVelocity;
    */

		//Debugging begin
    //if ( Step % 32 ==0 && RANK == MASTER_CPU ) {
  	//	printf("VX =  %f, VY =%f, VZ =%f , TimeSteps =%d\n",
    //          BoundaryConditions[0]->Data[0],
    //          BoundaryConditions[0]->Data[1],
    //          BoundaryConditions[0]->Data[2], 
    //          Step / 32 );
    // }
		//Debugging end
