...
double* BoundaryBuffer::getProtocol( double* Filed ) {
    this->updateProtocol( Filed );
    return m_Protocol;
}
...
int  BoundaryBuffer::updateProtocol( double* Filed ) {
	...
	for ( std::list<unsigned>::iterator Iterator = m_BufferElements.begin();
	      ...
	      m_Protocol[ Counter ] = Filed[ ( *Iterator ) ];
	}
