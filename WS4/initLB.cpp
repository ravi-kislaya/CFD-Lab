...
// for the ID Field
	...
	for ( unsigned z = 0; z < Length[ 2 ] + 2; ++z ) {
		for ( unsigned y = 0; y < Length[ 1 ] + 2; ++y ) {
			( *flagField )[ computeFlagIndex( 0, y, z, Length ) ] = PARALLEL_BOUNDARY;
			( *flagField )[ computeFlagIndex( Length[ 0 ] + 1, y, z, Length ) ] = PARALLEL_BOUNDARY;
		}
	}
	...
	for ( unsigned y = 0; y < Length[ 1 ] + 2; ++y ) {
		for ( unsigned x = 0; x < Length[ 0 ] + 2; ++x ) {
			( *flagField )[ computeFlagIndex( x, y, z, Length ) ] = PARALLEL_BOUNDARY;
			( *flagField )[ computeFlagIndex( x, y, Length[ 2 ] + 1, Length ) ] = PARALLEL_BOUNDARY;
		}
	}
	...
	for ( unsigned z = 0; z < Length[ 2 ] + 2; ++z ) {
		for ( unsigned x = 0; x < Length[ 0 ] + 2; ++x ) {
			( *flagField )[ computeFlagIndex( x, 0, z, Length ) ] = PARALLEL_BOUNDARY;
			( *flagField )[ computeFlagIndex( x, Length[ 1 ] + 1, z, Length ) ] = PARALLEL_BOUNDARY;
		}
	}
